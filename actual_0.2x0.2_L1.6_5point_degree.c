...
real cd_val [] = {0.01709, 0.01782, 0.01864, 0.01566, 0.01997, 0.02125, 0.02298,
       ...
       0.13804, 0.15233, 0.17413, 0.19587, 0.23071, 0.26041, 0.2759 ,
...
#define gamma_l  0.28*(1.4 - 6*(0.06 - (thickness/chord_len)))
#define gamma_d  0.28*(1 - 2.5*(0.06 - (thickness/chord_len))) 
...
#define alpha_ml(alpha,kappa,alpha_dot, relative_vel) alpha - (gamma_l*kappa*sqrt(fabs(0.5*chord_len*alpha_dot/relative_vel))*sign(alpha_dot))  /* can create a singularity issue */
#define alpha_md(alpha,kappa,alpha_dot, relative_vel) alpha - (gamma_d*kappa*sqrt(fabs(0.5*chord_len*alpha_dot/relative_vel))*sign(alpha_dot))  /* can create a singularity issue */
...
/* Calculating Torque and new tip speed ratio */
#define lambda_cal(torque) torque/(vel_inf*moi) 
...
//    real cellid[3];
    ...
    for (j = 0; j < wings ; j++)
    {
        if (compx(x[0], act_x[j]) && compy(x[1], act_y[j]))
        {
            ...
            real beta_last = beta(act_theta[j], u, v);
            ...
            real alpha_ml_act = alpha_ml(alpha_act, kappa_act, alpha_dot_act, v_rel_act);
            real alpha_md_act = alpha_ml(alpha_act, kappa_act, alpha_dot_act, v_rel_act);
            real Cl_act = Cl((int) alpha_ml_act);
...
//        // printf("%g\n", source);    
    ...
    torque_act = torque_act + source*x[1];
...
//        // printf("%g\n", source);    
    ...
    torque_act = torque_act + source*x[0];
