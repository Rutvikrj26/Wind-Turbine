...
//wing data
const real chord_len = 0.42;
const real thickness = 0.15*0.42;
#define wings 3
#define points 4
#define eta_vals [points]
...
#define x_new(r,theta,point) (r*cos(theta) + (point - 1)*(chord_approx/3)*sin(theta))
#define y_new(r,theta,point) (r*sin(theta) - (point - 1)*(chord_approx/3)*cos(theta))
...
#define rot_vel (vel_inf*lambda_inf/radius)
...
#define alpha_ml(alpha,kappa,alpha_dot, relative_vel) 57.32*(alpha - gamma_l*kappa*sqrt(fabs(0.5*chord_len*alpha_dot/relative_vel))*(sign(alpha_dot)))  /* can create a singularity issue */
#define alpha_md(alpha,kappa,alpha_dot, relative_vel) 57.32*(alpha - gamma_d*kappa*sqrt(fabs(0.5*chord_len*alpha_dot/relative_vel))*(sign(alpha_dot)))  /* can create a singularity issue */
...
/* Calculating Eta */
#define eta(x,y,point_x, point_y) pow(2.718,-(fabs(pow((x-point_x),2) + pow((y-point_y),2))*pow(epsilon,-2)))/(pow(epsilon,3)*pow(3.14,1.5))
...
#define compxnet(x_cent,x_act) (fabs(x_act - x_cent) < 0.75*radius)
#define compynet(y_cent,y_act) (fabs(y_act - y_cent) < 0.75*radius)



DEFINE_SOURCE(xmom_source, c, t, dS, eqn)
{
    // Initialising Source Value
    real source = 0;

    // Getting the location of the current cell   
    real x[ND_ND];

    //Getting the time and calculating timestep
    real time = CURRENT_TIME;
    real last_time = PREVIOUS_TIME;
    real timestep = (time - last_time);

    //Getting the X and Y velocities and calculating the net Velocity
    real u = C_U(c,t);
    real v = C_V(c,t);
    real vnet_act = v_net(u,v);

    real u_last = C_U_M1(c,t);
    real v_last = C_V_M1(c,t);             
    real v_net_last = v_net(u_last, v_last);

    //calculating the angles of wings
    real act_theta [wings];    
    real last_theta [wings];

    //Cacluating the Multi-Dimensional Array of Coordinates
    real coordinates [wings][points][2];
    real eta_vals [wings][points];
    int wing;
    int point;
    for (wing = 0; wing < wings; wing++)
    {
        act_theta[wing] = theta_inst(rot_vel,time) + wing*(2*3.14/wings);
        last_theta[wing] = theta_inst(rot_vel,last_time) + wing*(2*3.14/wings);

        for (point = 0; point < points; point++)    
        {
            real x_coord = x_new(radius, act_theta[wing], point);
            real y_coord = y_new(radius, act_theta[wing], point);
            coordinates[wing][point][0] = x_coord;
            coordinates[wing][point][1] = y_coord;
        }
    }

    C_CENTROID(x,c,t);
    //applying Force of the Central Shaft 
    ...
    {    
        for (point = 0; point < points; point++)    
        {
            if (compx(x[0], coordinates[wing][point][0]) && compy(x[1], coordinates[wing][point][1]))
            {
                real lbd_act = lambda(vnet_act);
                real beta_act = beta(act_theta[wing], u, v);
                ...
                if (act_theta[wing] > 6.28*floor(act_theta[wing]/6.28) - 3.14/2  && act_theta[wing] < 6.28*floor(act_theta[wing]/6.28) + 3.14/2) 
                    {
                        source = 0.2*(-Fn_act * cos (act_theta[wing]) + Ft_act * sin (act_theta[wing])) + source;
                    }
                else
                    {
                        source = 0.2*(Fn_act * cos (act_theta[wing]) + Ft_act * sin (act_theta[wing]))+ source; 
